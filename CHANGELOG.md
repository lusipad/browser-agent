...
- **技能侧边栏全要素内联编辑 (Inline Skill Editor in Sidepanel)** —
  - 侧边栏技能抽屉（`📋`）新增「▶ 运行」与「✏️ 编辑」双模式切换，详情顶部与卡片均提供快捷编辑入口。
  - 技能列表项新增 `✏️` 按钮，点击一键进入编辑。
  - 支持直接内联修改技能名称、Emoji 图标、详细描述。
  - 步骤清单支持修改操作意图（`intent`）、导航 URL、条件备注；支持上下调序（`▲` / `▼`）、单步删除（`✕`）与追加新步骤（`+ 添加步骤`）。
  - 参数变量支持修改 Key、Label、类型、默认值，支持新增与删除变量。
  - 支持在侧边栏直接配置定时调度（Cron）开关与执行频次。
  - 点击「保存修改」即时持久化到 `chrome.storage`，显示成功提示动画。
- **技能生成后即时唤起审阅与微调** —
  - 无论从对话任务中提取技能还是示教录制保存技能，完成后自动唤起该技能的编辑视图，方便第一时间审阅与微调。

### Changed / 变更

- **技能参数全面支持留空与智能体自主推导分析** —
  - 大模型提炼生成的技能变量一律默认为非必填（`required: false`），界面标签统一显示为绿灰色的 `[选填]`，移除令人焦虑的红色 `[必填]` 标记。
  - 用户留空的参数自动转换为 `[自主推导: 变量名]`，执行时由智能体结合当前网页 DOM、URL 及上下文自主推断决策，彻底解决用户“很多必填项没法填”的痛点。

### Fixed / 修复

- **设置页技能管理路由与定位修复** —
  - 修复从侧边栏点击“管理全部技能”或“在设置中打开 ↗”时错误停留在模型厂商页（providers）的问题。
  - 支持通过 URL 参数 `?tab=skills&skillId=...` 精确打开技能面板并自动高亮选中目标技能。
  - 设置页监听 `onSkillsChange` 事件，侧边栏保存修改后设置页实时自动同步。

### Testing / 测试

- 132 项单元测试（新增技能编辑持久化与非必填覆盖测试）、11 项真实 Chromium Playwright E2E 测试与生产构建全部通过。
