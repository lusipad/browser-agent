...
- **商店图标重画**：从通用「联系人」剪影换成品牌一致的机器人脸（蓝色渐变 + 白色头 + 靛蓝眼 + 天线/微笑），与宣传图 🤖 主题统一，16px 工具栏尺寸仍可辨识；仍由无依赖生成器逐尺寸绘制（16/32/48/128）。
- **上架资料补齐**：新增 440×280 小宣传图；`STORE_SUBMISSION.md` 按官方文档补全远程代码声明、分发设置、内容分级、主页/支持 URL 等字段。
