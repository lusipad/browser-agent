...
- **可见标识**：agent 操作过的每个标签都被归入蓝色「🤖 Agent」标签组，用户一眼可见哪些标签正被 AI 控制；点「⏏ 释放控制」即撤销分组并断开调试连接（叠加 Chrome 附加调试器时的原生横幅，双重可见）
...
- **多语言（i18n）**：界面中英双语，默认跟随浏览器语言，也可在设置里强制切换；轻量自建 `t()` + 拆分字典（`src/shared/i18n/`），后台面向用户的提示与授权卡片同样本地化；manifest 用 `_locales` 让商店名称/描述也多语言。agent 的对话回复本就跟随用户语言
