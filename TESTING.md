...
> 感知层与适配器已有**自动化测试**覆盖（`npm test`：30 单元 + 6 真实 Chromium E2E，含 Shadow DOM / iframe 穿透、坐标换算、form_input、适配器流式解析）。本文档覆盖的是**必须加载扩展**才能验证的全链路行为：CDP 可信输入、侧边栏、完整 agent 循环、逐站授权等。

以下改动需要你在真实 Chrome 里跑一遍确认。整套约 5–10 分钟，覆盖每一项硬化功能。翻车的 case 把「侧边栏那条工具的报错详情」或「扩展 Service Worker 控制台日志」贴回来即可定位。
