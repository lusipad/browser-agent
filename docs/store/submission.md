...
`alarms`（**定时调度技能**）
```
根据用户在设置中设定的定时调度策略（周期性或每日定时），通过 chrome.alarms 准时唤醒扩展后台并触发自动化技能任务执行。
```
`notifications`（**任务完成与介入提醒**）
```
当定时自动化任务或长程技能在后台静默执行完毕、或遇到滑块拼图等安全阻断需要人工协同介入时，向用户发送 Chrome 桌面系统通知。
```
`scripting`
```
向用户已授权的页面注入自包含的 DOM 感知与选区交互函数，实现局部视觉框选及收集可交互元素供模型定位与操作。
```
`tabs` / `tabGroups`
```
